 ...
 * category breakdown, stats, and paginated transaction list.
 */
...
export async function GET(
  ...
  try {
    ...
    const matching = allExpenses.filter((e) => e.vendorName.toLowerCase() === lowerTarget);
    ...
    const prevMatching = prevExpenses.filter((e) => e.vendorName.toLowerCase() === lowerTarget);
    ...
    if (count === 0) {
      return NextResponse.json({
        ...
        frequency: null,
        monthlyTrend: [], categoryBreakdown: [], expenses: [],
      });
    }
    ...
    for (const e of matching) {
      ...
      else catMap.set(e.category, { total: e.amount, count: 1 });
    }
    ...
    return NextResponse.json({
      ...
      expenses: paginatedExpenses.map((e) => ({
        ...
        category: e.category, notes: e.note || null,
