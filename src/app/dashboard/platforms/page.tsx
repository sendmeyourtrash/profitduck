...
import PlatformDetailTab from "@/components/platforms/PlatformDetailTab";
import { useDateRange } from "@/contexts/DateRangeContext";

const DOW_NAMES = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];

type TabKey = "overview" | "hourly" | "dow" | "fees" | "daily" | "detail";
...
const OVERVIEW_PLATFORM_LABELS: Record<string, string> = {
...
const OVERVIEW_PLATFORM_COLORS: Record<string, string> = {
  square: "#006aff",
  doordash: "#ff3008",
  ubereats: "#06c167",
  grubhub: "#ff8b00",
...
export default function AnalyticsPage() {
  ...
  // Read ?platform= param on mount to deep-link to a specific platform's detail view
  const initialPlatform = searchParams.get("platform");
  const [tab, setTab] = useState<TabKey>(initialPlatform && VALID_PLATFORMS.has(initialPlatform) ? "detail" : "overview");
  const [overviewPlatforms, setOverviewPlatforms] = useState<PlatformStats[]>([]);
  const [selectedPlatforms, setSelectedPlatforms] = useState<string[]>(
    initialPlatform && VALID_PLATFORMS.has(initialPlatform) ? [initialPlatform] : []
  );
  ...
  const [ubereatsHasTime, setUbereatsHasTime] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetches = [
      // Overview data from separate API
      (() => {
        const p = new URLSearchParams();
        if (globalStart) p.set("startDate", globalStart);
        if (globalEnd) p.set("endDate", globalEnd);
        return fetch(`/api/dashboard/platforms?${p}`).then(r => r.json()).then(data => {
          setOverviewPlatforms(data.platforms || []);
        });
      })(),
      fetchAnalytics("revenue_by_hour", selectedDow).then((data) => {
        setHourlyData(data.hourly || []);
        setHourlyDaysInSample(data.daysInSample || 1);
        setUbereatsHasTime(data.ubereatsHasTime ?? false);
      }),
  ...
  const TABS: { key: TabKey; label: string }[] = [
    ...
    { key: "detail", label: "Platform Detail" },
  ];
        ...
        <div className={`transition-opacity ${refreshing ? "opacity-60 pointer-events-none" : ""}`}>
          ...
          {tab === "overview" && (() => {
            const filtered = selectedPlatforms.length === 0
              ? overviewPlatforms
              : overviewPlatforms.filter(p => selectedPlatforms.includes(p.platform));
            if (filtered.length === 0) return (
              <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                <p className="text-lg">No platform data</p>
                <p className="mt-2">Import data or select different platforms.</p>
              </div>
            );
            const bestPlatform = [...filtered].sort((a, b) => b.netPayout - a.netPayout)[0];
            const mostEfficient = [...filtered].sort((a, b) => a.commissionRate - b.commissionRate)[0];
            return (
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <StatCard
                    title="Most Revenue"
                    value={OVERVIEW_PLATFORM_LABELS[bestPlatform.platform] || bestPlatform.platform}
                    subtitle={formatCurrency(bestPlatform.netPayout)}
                    variant="success"
                  />
                  <StatCard
                    title="Lowest Commission"
                    value={OVERVIEW_PLATFORM_LABELS[mostEfficient.platform] || mostEfficient.platform}
                    subtitle={`${mostEfficient.commissionRate}% commission rate`}
                    variant="success"
                  />
                  <StatCard
                    title="Total Orders"
                    value={filtered.reduce((s, p) => s + p.orderCount, 0).toLocaleString()}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <BarChartCard
                    ...
                    showPercentToggle
                    data={filtered.map(p => ({
                      name: OVERVIEW_PLATFORM_LABELS[p.platform] || p.platform,
                      value: p.netPayout,
                      color: OVERVIEW_PLATFORM_COLORS[p.platform],
                    }))}
                  />
                  <BarChartCard
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 p-6 pb-4">
                  <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">Platform Comparison</h3>
                      ...
                      <tbody>
                        {filtered.map(p => (
                          <tr
                            ...
                            onClick={() => { setSelectedPlatforms([p.platform]); setTab("detail"); }}
                          >
                            <td className="py-3 pl-4 pr-4 text-gray-800 dark:text-gray-200 font-medium whitespace-nowrap rounded-l-xl group-hover:bg-gray-50 dark:group-hover:bg-gray-700/30 transition-colors">
                              {OVERVIEW_PLATFORM_LABELS[p.platform] || p.platform}
                            </td>
                            <td className="py-3 px-3 text-right text-gray-600 dark:text-gray-400 whitespace-nowrap group-hover:bg-gray-50 dark:group-hover:bg-gray-700/30 transition-colors">{p.orderCount}</td>
                            <td className="py-3 px-3 text-right text-gray-800 dark:text-gray-200 whitespace-nowrap group-hover:bg-gray-50 dark:group-hover:bg-gray-700/30 transition-colors">{formatCurrency(p.grossRevenue)}</td>
                            <td className="py-3 px-3 text-right text-red-600 dark:text-red-400 whitespace-nowrap group-hover:bg-gray-50 dark:group-hover:bg-gray-700/30 transition-colors">{formatCurrency(p.totalFees)}</td>
                            <td className="py-3 px-3 text-right font-medium text-emerald-600 dark:text-emerald-400 whitespace-nowrap group-hover:bg-gray-50 dark:group-hover:bg-gray-700/30 transition-colors">{formatCurrency(p.netPayout)}</td>
                            <td className="py-3 px-3 text-right whitespace-nowrap group-hover:bg-gray-50 dark:group-hover:bg-gray-700/30 transition-colors">
                              ...
                              }`}>{p.commissionRate}%</span>
                            </td>
                            <td className="py-3 px-3 text-right text-gray-600 dark:text-gray-400 whitespace-nowrap group-hover:bg-gray-50 dark:group-hover:bg-gray-700/30 transition-colors">{formatCurrency(p.avgOrderValue)}</td>
                            <td className="py-3 pl-3 pr-4 text-right text-gray-600 dark:text-gray-400 whitespace-nowrap rounded-r-xl group-hover:bg-gray-50 dark:group-hover:bg-gray-700/30 transition-colors">{formatCurrency(p.tips)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          {tab === "hourly" && (
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200/50 dark:border-gray-700/50 p-6">
              {!ubereatsHasTime && (selectedPlatforms.length === 0 || selectedPlatforms.includes("ubereats")) && (
