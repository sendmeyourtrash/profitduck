/**
 * GrubHub ISOLATED world bridge — relays messages between MAIN world
...
(function () {
  ...
  window.addEventListener("message", (event) => {
    ...
    if (event.data.type === "PROFITDUCK_SEND_ORDERS" && event.data.platform === "grubhub") {
      // Route csvRows through background (its fetch isn't subject to page CORS)
      chrome.runtime.sendMessage({
        action: "send_grubhub_csvrows",
      ...
      chrome.runtime.sendMessage({ action: "get_known_ids", platform: "grubhub" }, (response) => {
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === "local" && changes.syncRequest?.newValue) {
      ...
      if (req.platform && req.platform !== "grubhub" && req.platform !== "all") return;
  ...
  console.log("[Profit Duck] GrubHub bridge active");
})();
